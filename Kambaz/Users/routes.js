...
export default function UserRoutes(app) {
  ...
  const deleteUserEnrollment = (req, res) => {
    const { enrollmentId } = req.params;
    enrollmentsDao.deleteEnrollment(enrollmentId);
    res.sendStatus(200);
  };
  const addUserEnrollment = (req, res) => {
    let { userId } = req.params;
    if (userId === "current") {
      const currentUser = req.session["currentUser"];
      if (!currentUser) {
    ...
    enrollmentsDao.addEnrollment({...req.body, user: userId});
    res.sendStatus(200);
  };
  app.post("/api/users/:userId/:courseId/enrollments", addUserEnrollment);
  app.delete("/api/users/enrollments/:enrollmentId", deleteUserEnrollment);
